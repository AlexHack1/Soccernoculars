# in terminal, pip install the following before running, do not uncomment
...
def player_add(player_link,threesixfive,year,position,mega_list):
    ...
    if bottom(lambda t: 'Scouting' in t.text):
        ...
        scout_report = uls[-2]
        # print(scout_report)
        seasons = scout_report.find_all('a')
        # print(seasons)
        k = 1
        ks = []
...
def extract_players(link, myposition,threesixfive,year):
        ...
        for tr in table.find_all('tr'):
            ...
            mega_frame.to_csv(r'%s'%file_path,sep= ',',header = False,index = False,index_label=False, mode='a',encoding='utf8')
...
file_path = input(f'Enter a full csv file path to save the data to:')
...
# transpose option
print('currently, players stats occupy rows in the spreadsheet. \n Would you like to transpose the dataset so that they occupy columns instead?')

transpose_bool = input('y to transpose')
if transpose_bool == 'y':
    pd.read_csv(r'%s'%file_path,sep= ',',header=None,engine='python').T.to_csv(r'%s'%file_path, sep= ',', header=False, index=False,engine='python')
print('all done!')